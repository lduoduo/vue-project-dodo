...
module.exports = (app, cb) => {
  ...
  const apiOutDir = appConfig.output.path
  let isFrist = true
  ...
  // server for api
  appConfig.entry.app = ['webpack-hot-middleware/client?path=/__webpack_hmr&timeout=2000&reload=true', appConfig.entry.app]
  appConfig.plugins.push(
    new webpack.HotModuleReplacementPlugin(),
    new webpack.NoEmitOnErrorsPlugin()
  )
  const apiCompiler = webpack(appConfig);
  ...
  apiCompiler.watch({}, (err, stats) => {
    ...
    console.log('start-server-dev...')

    apiMfs.readdir(path.join(__dirname, '../dist/server'), function (err, files) {
      if (err) {
    ...
    apiMain = apiMfs.readFileSync(path.join(appConfig.output.path, 'app.js'), 'utf-8')
    update()
  })
